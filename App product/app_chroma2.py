...
def search():
    ...
    return render_template('results_page.html', results=grouped_results)
...
def search():
    ...
    # Merge results: prioritize keyword matches, then embedding results
    combined_results = keyword_matches + [
        obj for obj in embedding_data if obj not in keyword_matches
    ]

    # Store search history
    session['search_history'].append({
        'query': query_text,
        'results': combined_results[:5]  # Limit displayed results
    })
    session.modified = True
